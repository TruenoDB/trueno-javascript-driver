...
class Vertex extends Component {
  ...
  in(cmp, ftr) {
    ...
    if(this.__debug){
      console.log('DEBUG[in]: ', apiFunc, msg);
    }

    /* return promise with the async operation */
    return new Promise((resolve, reject)=> {
      self.__conn._rpc.call(apiFunc, msg).then((msg)=> {
        resolve(msg.payload);
      }, (err)=> {
        reject(err);
      });
  ...
  inDegree(cmp, ftr) {
    ...
    /* if debug display operation params */
    if(this.__debug){
      console.log('DEBUG[inDegree]: ', apiFunc, msg);
    }
  ...
  outDegree(cmp, ftr) {
    ...
    /* if debug display operation params */
    if(this.__debug){
      console.log('DEBUG[outDegree]: ', apiFunc, msg);
    }
