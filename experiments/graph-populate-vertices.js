...
if( !(totalArguments==requiredArguments) ) {
  console.log("[usage] node graph-populate-vertices.js graph [host] [port]");
  process.exit(0)
}

const destinationGraph = process.argv[2].toString();

/* Elastic Search Index used */
let indexName = destinationGraph;
let typeName = "v";

/* Validating IP address */
if( totalArguments >= requiredArguments + 1){
  let tmpAddress = new ipAddress(process.argv[3].toString());
  /* If IP address is valid override default address */
  if(tmpAddress.isValid()){
    strHost = process.argv[3].toString();
  }
  else{
...
if( (totalArguments == (requiredArguments + 2)) ){
  strPort = process.argv[4].toString();
}
...
const vertices = require("./datasets/" + destinationGraph + "/" + destinationGraph + "-vertices.json");
...
function insertDeleteVertices(arr, op) {
  ...
  _bulk().then((result) => {
    /* computing progressbar thickness */
